...
function FileUploader() {
    ...
    const handleUpload = async () => {
      ...
      try {
          const response = await axios.post("http://127.0.0.1:5000/process-subtitles", formData, {
              headers: {
          ...
          setFilename(file.name.endsWith(".srt") ? file.name : `${file.name.split(".")[0]}_edited.srt`);
      } catch (error) {
    ...
    return (
        <div>
            <input type="file" accept=".srt,.wav,.mp3,.mp4,.m4a" onChange={handleFileChange} />
